...
DESCRIPTION
===========
**slacube ped-mon** repeatedly takes short pedestal runs to monitor pedestal drift over long periods, and analyzes/plots the accumulated results. It works under `$SLACUBE_WORKDIR` and requires `$CTRL_FILE`, `$BAD_CHANNEL_FILE`, and `$SLACUBE_DROPBOX` for `start`. For commands with _file_, either provide a file path, or choose from an interactive list of files under `$SLACUBE_DROPBOX`.
...
SEE ALSO
========
`slacube help`, `slacube env help`, `slacube pedestal help`
